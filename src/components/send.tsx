...
export default function SendTransection() {
  ...
  const formSchema = z.object({
    receiversChain: z
      .string()
      .refine((val) =>
        ['mumbai', 'avalanche', 'scroll', 'mantle'].includes(val)
      ),
    receiverAddress: z
      .string()
      .min(42)
  ...
  async function handleOnSubmit(data: z.infer<typeof formSchema>) {
    ...
    setIsLoading(true)
    try {
      await executeSendToken(
        data.receiverAddress,
        data.receiversChain,
        data.token,
        parseInt(data.tokenFromChains.avalanche),
        parseInt(data.tokenFromChains.mumbai)
      )
      toast({
        title: 'Success',
      ...
      toast({
        ...
        title: 'Error',
        description:
          'There was some error while sending the multichain transection, try again later!',
      })
    }
    setIsLoading(false)
  }
      ...
      <form onSubmit={form.handleSubmit(handleOnSubmit)}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select the chain and amount of tokens needed to send, MCSquare
              Wallet can make cross chain transactions as well.
            </CardDescription>
          </CardHeader>
                        ...
                        <SelectItem value='aUSDC'>
                          <div className='w-full flex justify-around items-center'>
                            ...
                            <div className='flex flex-col items-start ml-4'>
                              ...
                              <p className='text-md text-muted-foreground'>
                                Available: {walletBalances?.aUSDC.totalBalance}{' '}
                                aUSDC
                              </p>
                            </div>
                          </div>
                      ...
                      <div className='grid gap-2'>
                          ...
                          render={({ field }) => {
                            return (
                              <FormItem className='grid grid-cols-3 items-center gap-4'>
                                <FormLabel htmlFor='mumbaiAmount'>
                                  Mumbai
                                </FormLabel>
                                <Input
                                  {...field}
                        ...
                        <FormField
                          ...
                          render={({ field }) => {
                            return (
                              <FormItem className='grid grid-cols-3 items-center gap-4'>
                                <FormLabel htmlFor='avalancheAmount'>
                                  Avalanche
                                </FormLabel>
                                <Input
                                  {...field}
                        ...
                        <FormField
                          ...
                          render={({ field }) => {
                            return (
                              <FormItem className='grid grid-cols-3 items-center gap-4'>
                                <FormLabel htmlFor='scrollAmount'>
                                  Scroll
                                </FormLabel>
                                <Input
                                  {...field}
                        ...
                        <FormField
                          ...
                          render={({ field }) => {
                            return (
                              <FormItem className='grid grid-cols-3 items-center gap-4'>
                                <FormLabel htmlFor='mantleAmount'>
                                  Mantle
                                </FormLabel>
                                <Input
                                  {...field}
